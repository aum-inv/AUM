...
{
    public partial class SmartCandleChart : BaseChartControl
    {
          
        public List<SmartCandleData> ChartData
        {
            get;
            set;
        }
        ...
        public override void View()
        {
            ...
            foreach (var item in ChartData)
            {
                int idx = chart.Series[0].Points.AddXY(item.DTime
                    , item.BasicPrice_High, item.BasicPrice_Low, item.BasicPrice_Open, item.BasicPrice_Close);
        ...
        public void DisplayView()
        {
            ...
            if (viewLists != null)
            {
                ...
                double maxPrice = viewLists.Max(m => m.BasicPrice_High);
                double minPrice = viewLists.Min(m => m.BasicPrice_Low);
        ...
        public void MoveView(SmartCandleData data)
        {
            List<SmartCandleData> viewLists = null;
            int maxDisplayIndex = 0;
            int minDisplayIndex = 0;
        ...
        public void DisplayChartLabel(SmartCandleData data, Color color, string type, string title = "▼")
        {
            foreach (var p in chart.Series[0].Points)
            {               
                if (p.Tag as SmartCandleData == data)
                {
