...
{
    public partial class PatternCandleChart : BaseChartControl
    {
        ...
        public override void View()
        {
            ...
            foreach (var item in ChartData)
            {
                int idx = chart.Series[0].Points.AddXY(item.DTime
                    , item.HighPrice, item.LowPrice, item.OpenPrice, item.ClosePrice);
        ...
        public void MoveView(DateTime date)
        {
            List<S_CandleItemData> viewLists = null;
            int maxDisplayIndex = 0;
            int minDisplayIndex = 0;
        ...
        public void DisplayChartLabel(DateTime dt, Color color, string type, string title = "▼")
        {
            foreach (var p in chart.Series[0].Points)
            {
                if ((p.Tag as S_CandleItemData).DTime == dt)
                {
                    TextAnnotation a = new TextAnnotation();
                    a.Font = new Font("굴림", 12F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
                    a.Text = title;
