...
{
    public partial class PatternTradeRuleForm2 : Form
    {
        BasicPatternTradeRule rule = new BasicPatternTradeRule(false);

        public PatternTradeRuleForm2()
        {
            InitializeComponent();
            this.Load += TradeRuleForm_Load;
            this.FormClosing += PatternTradeRuleForm2_FormClosing;
            SiseEvents.Instance.SiseHandler += Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler2 += Instance_TradeRuleHandler;
        }

        private void PatternTradeRuleForm2_FormClosing(object sender, FormClosingEventArgs e)
        {
            rule.Close();
            SiseEvents.Instance.SiseHandler -= Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler2 -= Instance_TradeRuleHandler;
        ...
        private void tbP1BuyPrice1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    tbP1BuyPrice2.Text = PriceTick.GetDownPriceOfTick(rule.Item, rule.P1BuyPrice1, buyTicks).ToString();
                }
