...
{
    public partial class PatternTradeRuleForm3 : Form
    {
        BasicPatternTradeRule2 rule = new BasicPatternTradeRule2(true);

        public PatternTradeRuleForm3()
        {
            InitializeComponent();
            this.Load += TradeRuleForm_Load;
            this.FormClosing += PatternTradeRuleForm3_FormClosing;
            SiseEvents.Instance.SiseHandler += Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler2 += Instance_TradeRuleHandler;
        }

        private void PatternTradeRuleForm3_FormClosing(object sender, FormClosingEventArgs e)
        {
            rule.Close();
            SiseEvents.Instance.SiseHandler -= Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler2 -= Instance_TradeRuleHandler;
        ...
        private void tbP2BuyPrice2_TextChanged(object sender, EventArgs e)
        {
            ...
            rule.P2BuyPrice1[1] = Convert.ToDouble(tbP2BuyPrice2.Text);
        }

        private void tbP2BuyPrice3_TextChanged(object sender, EventArgs e)
        {
            if (tbP2BuyPrice3.Text.Length == 0) return;
            rule.P2BuyPrice1[2] = Convert.ToDouble(tbP2BuyPrice3.Text);
        }
        private void tbP2BuyPrice1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    tbP2BuyPrice2.Text = PriceTick.GetUpPriceOfTick(rule.Item, rule.P2BuyPrice1[0], 5).ToString();
                }
                if (tbP2BuyPrice3.Text == "0")
                {
                    tbP2BuyPrice3.Text = PriceTick.GetUpPriceOfTick(rule.Item, rule.P2BuyPrice1[0], 10).ToString();
                }
                ...
                {
                    tbMinimumRevenue2.Text = PriceTick.GetUpPriceOfTick(rule.Item, rule.P2BuyPrice2[2],  revenueTick).ToString();
                }
            }
        }
        ...
        private void tbP1BuyPrice2_TextChanged(object sender, EventArgs e)
        {
            ...
            rule.P1BuyPrice1[1] = Convert.ToDouble(tbP1BuyPrice2.Text);
        }

        private void tbP1BuyPrice3_TextChanged(object sender, EventArgs e)
        {
            if (tbP1BuyPrice3.Text.Length == 0) return;
            rule.P1BuyPrice1[2] = Convert.ToDouble(tbP1BuyPrice3.Text);
        }
