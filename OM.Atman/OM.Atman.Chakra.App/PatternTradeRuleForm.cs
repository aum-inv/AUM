...
{
    public partial class PatternTradeRuleForm : Form
    {
        BasicPatternTradeRule rule = new BasicPatternTradeRule(true);

        public PatternTradeRuleForm()
        {
            InitializeComponent();
            this.Load += TradeRuleForm_Load;
            this.FormClosing += PatternTradeRuleForm_FormClosing;
            SiseEvents.Instance.SiseHandler += Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler2 += Instance_TradeRuleHandler;
        }

        private void PatternTradeRuleForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            rule.Close();
            SiseEvents.Instance.SiseHandler -= Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler2 -= Instance_TradeRuleHandler;
        ...
        {
            //if (atmanName != "PATTERNTRADE1") return;
        ...
        private void tbP2BuyPrice1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {                
                ...
                {
                    tbP2LossPrice1.Text = PriceTick.GetDownPriceOfTick(rule.Item, rule.P2BuyPrice1, lossTick1).ToString();
                }
                if (tbP2LossPrice2.Text == "0")
                {
                    tbP2LossPrice2.Text = PriceTick.GetDownPriceOfTick(rule.Item, rule.P2BuyPrice1, lossTick2).ToString();
                }
                if (tbMinimumRevenue2.Text == "0")
                {
                    tbMinimumRevenue2.Text = PriceTick.GetUpPriceOfTick(rule.Item, rule.P2BuyPrice1,  revenueTick).ToString();
                }
            }
        }
        ...
        private void tbP1BuyPrice1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {                
                ...
                {
                    tbP1LossPrice1.Text = PriceTick.GetUpPriceOfTick(rule.Item, rule.P1BuyPrice1, lossTick1).ToString();
                }
                if (tbP1LossPrice2.Text == "0")
                {
                    tbP1LossPrice2.Text = PriceTick.GetUpPriceOfTick(rule.Item, rule.P1BuyPrice1, lossTick2).ToString();
                }
                if (tbMinimumRevenue1.Text == "0")
                {
                    tbMinimumRevenue1.Text = PriceTick.GetDownPriceOfTick(rule.Item, rule.P1BuyPrice1, revenueTick).ToString();
                }
            }
        }
