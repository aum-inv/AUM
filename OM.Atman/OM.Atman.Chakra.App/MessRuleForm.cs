...
{
    public partial class MessRuleForm : Form
    {
        List<MessTradeRule> ruleList = new List<MessTradeRule>();

        public MessRuleForm()
        {
            InitializeComponent();

            this.FormClosing += MessRuleForm_FormClosing;
            SiseEvents.Instance.SiseHandler += Instance_SiseHandler;
            TradeEvents.Instance.TradeRuleHandler += Instance_TradeRuleHandler;
        }

        private void MessRuleForm_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            {
                if (item.RuleID.Contains("Atom"))
                {
                    int idx = dgv.Rows.Add(item.RuleID, item.Item, item.Position, item.BasePrice, item.IsUse == "Y", item.IsBuyDone);
                    dgv.Rows[idx].Tag = item;

                    ruleList.Add(new MessTradeRule(item));
                }
            }
        }
